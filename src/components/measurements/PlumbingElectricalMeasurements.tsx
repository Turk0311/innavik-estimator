  ...
  return (
    <div>
      <label className={labelCls}>
        {label}{hint && <span className="text-slate-500 ml-1">({hint})</span>}
      </label>
      <input
        type="number"
        value={value === 0 ? "" : value}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-4">
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Electrical</h3>
      ...
      {/* Plumbing */}
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5 space-y-4">
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Plumbing</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <Field label="Rough-in Plumbing" hint="count" value={data.roughInPlumbing} onChange={(v) => update("roughInPlumbing", v)} />
          <Field label="Plumbing Fixtures" hint="count" value={data.plumbingFixtures} onChange={(v) => update("plumbingFixtures", v)} />
        </div>
        {plumbingHours > 0 && (
          <div className="rounded-lg bg-blue-900/20 border border-blue-800/50 px-4 py-2.5 flex items-center gap-2">
            <svg className="w-4 h-4 text-blue-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
        ...
        <textarea
          ...
          rows={3}
          className="w-full rounded-lg bg-slate-700 border border-slate-600 text-white placeholder-slate-500 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none"
        />
      </div>
    </div>
  );
